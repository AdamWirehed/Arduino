...
void loop()                                  // Main loop auto-repeats
{
  ...
  if((wLeft == 0) && (wRight == 0))          // If both whiskers contact
  {
    ...
    turnLeft(random(133,1200));                           // Turn left between 20 and 180 degrees
  }
  else if(wLeft == 0)                        // If only left whisker contact
  {
    backward(1000);                          // Back up 1 second
    turnRight(random(133,1200));                          // Turn right between 20 and 180 degrees degrees
  }
  else if(wRight == 0)                       // If only right whisker contact
  {
    backward(1000);                          // Back up 1 second
    turnLeft(random(133,1200));                           // Turn left between 20 and 180 degrees degrees
  }
